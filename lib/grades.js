...
const getGrade = async (userData, tokenData = null) => {
    try {
        ...
        const divThresholds = divisionThresholdsCall.data.data;
        ...
        let userIsInRooftop = false;
        if (currentGrade === undefined || currentGrade === grades.purple || currentGrade === grades.red || currentGrade == grades.gold) {
            if (!tokenData) tokenData = await getAccessToken(null, userData);
            const userGuilds = await callEndpoint(endpoints.guilds, tokenData);
            userIsInRooftop = userGuilds.data.find(guild => guild.id === '798383083375951952') != undefined;
        }
        ...
        if (userIsInRooftop) {
            ...
            if (userGuildMemberCall.data.roles.includes(MMP)) {
                ...
                return currentGrade !== grades.red ? 
                    ...
                    grading(grades.red, "You must face the council and be accepted as a champion to be granted the next belt...", true);
            }
        }
        ...
        if (score >= divThresholds.two) {
            ...
            return currentGrade !== grades.blue ? 
            ...
            return currentGrade !== grades.green ? 
                grading(grades.green, "Your show immense growth. ```ansi\nDice Sensei hands you a \u001B[0;32mGreen Belt```", true) :
                grading(grades.green, "You must climb to fourth division to be granted the next belt", false);
        }
        else {
            return currentGrade ? 
                ...
                grading(grades.white, "You must climb to sixth division to be granted the next belt", false);
        }
    } catch (err) {
        console.error(err);
...
const assignGrade = (member, userData, grading) => {
    if (grading.isNewGrade) {
        try {
            Object.values(grades).forEach(val => {
                if (member._roles.some(snowflake => snowflake === val)) {
                    member.roles.remove(val);
                }
            });

            member.roles.add(grading.grade);
            userData.currentGrade = grading.grade;
